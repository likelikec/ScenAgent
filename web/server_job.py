...
def verify_session(session_id: str) -> bool:
    """Verify session_id against the central auth service. Returns True if valid."""
    try:
...
def api_auth_session(request: Request):
    ...
    return ok({"session_id": sid, "valid": True})


@app.get("/api/auth/user")
def api_auth_user(request: Request):
    """Transparent proxy to central auth — returns the raw user object from 8980."""
    sid = get_session_id(request)
    if not sid:
